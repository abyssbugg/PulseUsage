...
/// Proxy configuration loaded from ~/.pulseusage/config.json
...
/// Config file path: ~/.pulseusage/config.json
fn config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".pulseusage").join("config.json"))
}

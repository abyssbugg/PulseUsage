...
tauri_panel! {
    ...
    panel_event!(PulseUsagePanelEventHandler {
...
pub fn init(app_handle: &tauri::AppHandle) -> tauri::Result<()> {
    ...
    let panel = window.to_panel::<PulseUsagePanel>()?;
    ...
    let event_handler = PulseUsagePanelEventHandler::new();

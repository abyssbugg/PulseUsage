...
// Helper to create a valid JWT with configurable payload
function makeJwt(expOrPayload) {
  const header = btoa(JSON.stringify({ alg: "HS256", typ: "JWT" }))
  const payloadObject = typeof expOrPayload === "object"
    ? { org_id: "org_123", email: "test@example.com", ...expOrPayload }
    : { exp: expOrPayload, org_id: "org_123", email: "test@example.com" }
...
describe("factory plugin", () => {
  ...
  it("formats Factory UI-style rich usage metrics", async () => {
    const ctx = makeCtx()
    const futureExp = Math.floor(Date.now() / 1000) + 7 * 24 * 60 * 60
    ctx.host.fs.writeText("~/.factory/auth.json", JSON.stringify({
      access_token: makeJwt(futureExp),

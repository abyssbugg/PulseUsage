(function () {
  ...
  function fetchUsage(ctx, accessToken) {
    ...
    var resp = requestUsage(ctx, accessToken, "POST", body)
    if (resp.status === 405) {
      ctx.host.log.info("POST returned 405, retrying with GET")
      resp = requestUsage(ctx, accessToken, "GET", body)
    }
    return resp
  }
  ...
  function metricEndValue(metric, fallbackEnd) {
    const value = firstValue(metric, ["resetsAt", "resetAt", "endDate", "endAt", "periodEnd", "periodEndDate", "windowEnd"])
  ...
  globalThis.__pulseusage_plugin = { id: "factory", probe }
})()

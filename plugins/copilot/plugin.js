(function () {
  const KEYCHAIN_SERVICE = "PulseUsage-copilot";
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed && parsed.token) {
          ctx.host.log.info("token loaded from PulseUsage keychain");
          return { token: parsed.token, source: "keychain" };
        }
      }
    } catch (e) {
      ctx.host.log.info("PulseUsage keychain read failed: " + String(e));
    }
    return null;
  }
  ...
  function probe(ctx) {
    ...
    // Persist gh-cli token to PulseUsage keychain for future use
    if (source === "gh-cli") {
      saveToken(ctx, token);
    }
  ...
  globalThis.__pulseusage_plugin = { id: "copilot", probe };
})();

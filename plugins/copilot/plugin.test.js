...
function setKeychainToken(ctx, token) {
  ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
    if (service === "PulseUsage-copilot") return JSON.stringify({ token });
    return null;
  });
}
...
describe("copilot plugin", () => {
  ...
  it("loads token from PulseUsage keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("does not persist token loaded from PulseUsage keychain", async () => {
    const ctx = makePluginTestContext();
  ...
  it("retries with gh-cli token when cached keychain token is stale", async () => {
    ...
    expect(ctx.host.keychain.deleteGenericPassword).toHaveBeenCalledWith("PulseUsage-copilot");
  ...
  it("falls back when PulseUsage keychain payload lacks token field", async () => {
    const ctx = makePluginTestContext();
    ctx.host.keychain.readGenericPassword.mockImplementation((service) => {
      if (service === "PulseUsage-copilot") return JSON.stringify({ notToken: "x" });

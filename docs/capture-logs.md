...
Use this when PulseUsage is not working and you need to share debug info.
...
1. In your macOS menu bar, find the PulseUsage icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If PulseUsage does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.abyssbugg.pulseusage
```
...
1. Attach `pulseusage.log`.
2. If you also see files like `pulseusage.log.1`, attach those too.

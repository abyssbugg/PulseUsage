...
1. **PulseUsage Keychain** (`PulseUsage-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the PulseUsage keychain for faster access on subsequent probes.

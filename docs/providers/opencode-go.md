...
PulseUsage reads the local OpenCode SQLite database directly:
...
PulseUsage uses the current published OpenCode Go plan limits from the official docs:
...
The public provider identity stays `opencode-go`. If OpenCode later exposes account-truth usage by API key, PulseUsage can swap the backend without changing the provider ID or UI contract.

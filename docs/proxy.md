...
PulseUsage can route provider and plugin HTTP requests through an optional proxy.

- Supported proxy types: `socks5://`, `http://`, `https://`
- Config file: `~/.pulseusage/config.json`
- Default: off
- UI: none

## Config File

Create `~/.pulseusage/config.json`:
...
- Restart PulseUsage after changing the file.
...
This applies to provider and plugin HTTP requests that go through PulseUsage's built-in HTTP client.

...
import { RELEASES_URL, buildCommitUrl, buildPullRequestUrl } from "@/lib/app-identity"
    ...
    return parts.map((part, i) => {
      ...
      if (part.type === "pr") {
        return (
          <button
            key={i}
            onClick={() => openUrl(buildPullRequestUrl(Number(part.content.slice(1)))).catch(console.error)}
            className={linkClass}
          >
            {part.content}
          </button>
      ...
      if (part.type === "commit") {
        return (
          <button
            key={i}
            onClick={() => openUrl(buildCommitUrl(part.content)).catch(console.error)}
